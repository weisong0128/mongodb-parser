 ...
 */
public class RemovalParser {
    private static final Logger logger = LoggerFactory.getLogger(RemovalParser.class);
//    private static final String originPath = System.getProperty("user.dir") + File.separator  +  "log_result.txt";
//    private static final String goalPath = System.getProperty("user.dir") + File.separator  +  "removal_result.txt";
...
//                    logger.info("{}");
                ...
                if (jsonObject.getJSONObject("query") != null) {
                    ...
                    if (jsonObject.getJSONObject("query").getString("sort") != null) {
                        ...
                        String sortJson = mapToJson(sortMap);
                        sb.append(",").append(sortJson);
                    }

                    //将展示字段projection中数据存入projectionMap
                    if (jsonObject.getJSONObject("query").getString("projection") != null) {
                        ...
                        String projectionJson = mapToJson2("projection", projectionMap);
                        sb.append(",").append(projectionJson);
                    }
...
//                    logger.info(sb.toString());
                    FileUtils.writeStringToFile(new File(goalPath), lineData + "\t" + sb.toString(), "UTF-8", true);
                }
                        ...
                        if (jsonObject.getJSONObject("command").getString("query") != null) {
                            ...
                            for (Map.Entry<String, Object> entry : queryObject.entrySet()) {
                                ...
                                if (key.contains("$or") || key.contains("$and")) {
                                    ...
                                    orMap.put(entry.getKey(), orFieldMap);
                    ...
                    if (jsonObject.getJSONObject("command").getString("aggregate") != null) {
                        ...
                        if (jsonObject.getJSONObject("command").getString("pipeline") != null) {
                            ...
                            for (int i = 0; i < jsonArray.size(); i++) {
                                ...
                                //7.关联情况去重
                                if (temp.containsKey("$lookup")) {
                                    Map<String, String> lookMap = new HashMap<>();
                                    JSONObject lookup = temp.getJSONObject("$lookup");
                                    for (Map.Entry<String, Object> entry : lookup.entrySet()) {
                                        ...
                                        if ("as".equals(key)) {
                                            ...
                                            lookMap.put(key, value);
                                        }
                                    }
                                    String lookJson = mapToJson("$lookup", lookMap);
                                    lookJson = "{" + lookJson + "}";
                                    JSONObject newTemp = JSON.parseObject(lookJson, Feature.OrderedField);
                                    Collections.replaceAll(jsonArray, temp, newTemp);
                                }
                        ...
                        num++;
//                        logger.info(sb.toString());
                        FileUtils.writeStringToFile(new File(goalPath), lineData + "\t" + sb.toString().replaceAll("\\\\", ""), "UTF-8", true);
                ...
                if (num == 0) {
//                    logger.info(sql);
                    FileUtils.writeStringToFile(new File(goalPath), lineData + "\t" + sql + "\r\n", "UTF-8", true);
                }
